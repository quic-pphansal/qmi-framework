...
atomic_int qcci_loglevel = ATOMIC_VAR_INIT(QCCI_LOG_ERR);
...
static void qcci_set_loglevel(const char *val)
{
    ...
    if (!strcasecmp(val, "NONE"))
        atomic_store(&qcci_loglevel, QCCI_LOG_NONE);
    else if (!strcasecmp(val, "ERR"))
        atomic_store(&qcci_loglevel, QCCI_LOG_ERR);
    else if (!strcasecmp(val, "WARN"))
        atomic_store(&qcci_loglevel, QCCI_LOG_WARN);
    else if (!strcasecmp(val, "INFO"))
        atomic_store(&qcci_loglevel, QCCI_LOG_INFO);
    else if (!strcasecmp(val, "DBG"))
        atomic_store(&qcci_loglevel, QCCI_LOG_DBG);
    else if (!strcasecmp(val, "TRACE"))
        atomic_store(&qcci_loglevel, QCCI_LOG_TRACE);
}
...
void qcci_log_write(qcci_log_level_t lvl, const char *fmt, ...)
{
    ...
    if (lvl > atomic_load(&qcci_loglevel))
        return;
