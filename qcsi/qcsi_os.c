...
atomic_int qcsi_loglevel = ATOMIC_VAR_INIT(QCSI_LOG_ERR);
...
static void qcsi_set_loglevel(const char *val)
{
    ...
    if (!strcasecmp(val, "NONE"))
        atomic_store(&qcsi_loglevel, QCSI_LOG_NONE);
    else if (!strcasecmp(val, "ERR"))
        atomic_store(&qcsi_loglevel, QCSI_LOG_ERR);
    else if (!strcasecmp(val, "WARN"))
        atomic_store(&qcsi_loglevel, QCSI_LOG_WARN);
    else if (!strcasecmp(val, "INFO"))
        atomic_store(&qcsi_loglevel, QCSI_LOG_INFO);
    else if (!strcasecmp(val, "DBG"))
        atomic_store(&qcsi_loglevel, QCSI_LOG_DBG);
    else if (!strcasecmp(val, "TRACE"))
        atomic_store(&qcsi_loglevel, QCSI_LOG_TRACE);
}
...
void qcsi_log_write(qcsi_log_level_t lvl, const char *fmt, ...)
{
    ...
    if (lvl > atomic_load(&qcsi_loglevel))
        return;
